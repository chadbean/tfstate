...
EXIT_WITH_ERROR = 1
EXIT_NORMALLY = 0
VIRTUALENV_PROMPT_TEMPLATE = "(%s) "
DEFAULT_SETTINGS_FILENAME = "vsettings.json"

DEFAULT_SETTINGS = dict(
    package_name="" or os.path.basename(os.path.abspath('.')),
    use_site_packages=False,
    virtualenv_dir="./.vs.env/",
    env_types=['pip'], #Possible values command_list, pip, buildout
...
def make_current_settings(settings_filename, default_settings=None):
    ...
    package_name = user_defined_settings.get('package_name',
                                             DEFAULT_SETTINGS.get('package_name'))
...
def quick_activation_script(virtualenv_dir, file="quickactivate",
        base_path='./'):
